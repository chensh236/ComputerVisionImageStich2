...
{
  VlKDForest * self = (VlKDForest*) vl_calloc (sizeof(VlKDForest), 1) ;
...
{
  VlKDForestSearcher * self = (VlKDForestSearcher*) vl_calloc(sizeof(VlKDForestSearcher), 1);
  ...
  self->searchHeapArray = (VlKDForestSearchState*) vl_malloc (sizeof(VlKDForestSearchState) * kdforest->maxNumNodes) ;
  self->searchIdBook = (vl_uindex*) vl_calloc (sizeof(vl_uindex), kdforest->numData) ;
  return self ;
}
...
vl_kdforest_build (VlKDForest * self, vl_size numData, void const * data)
{
  ...
  searchBounds = (double*) vl_malloc(sizeof(double) * 2 * self->dimension);
